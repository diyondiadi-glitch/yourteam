...
import { X, Eye, ThumbsUp, MessageSquare, Sparkles, Skull, RefreshCw, Zap, Scissors, PickaxeIcon } from "lucide-react";
...
export default function VideoModal({ video, avgViews, onClose }: VideoModalProps) {
  ...
  useEffect(() => {
    ...
    callAI(
      ...
      `Video: "${video.title}"\nViews: ${video.viewCount} (channel avg: ${Math.round(avgViews)})\nLikes: ${video.likeCount}\nComments: ${video.commentCount}\nPublished: ${video.publishedAt}`
    ).then(setInsight).catch(() => setInsight("AI analysis temporarily unavailable.")).finally(() => setInsightLoading(false));
  }, [video?.id]);
  ...
  function navWith(path: string) {
    if (video) {
      setSelectedVideo({ id: video.id, title: video.title, thumbnail: video.thumbnail, views: video.viewCount, likes: video.likeCount, comments: video.commentCount, publishDate: video.publishedAt });
    }
    onClose();
    navigate(path);
  }
  ...
  const perfPercent = avgViews > 0 ? Math.round(((video.viewCount - avgViews) / avgViews) * 100) : 0;
  const isAbove = perfPercent >= 0;
  const likeRate = video.viewCount > 0 ? ((video.likeCount / video.viewCount) * 100).toFixed(1) : "0";
      ...
      >
        ...
        >
          ...
          <div className="p-5 space-y-5">
            ...
            {/* Performance Badge */}
            <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold ${isAbove ? "bg-success/15 text-success border border-success/20" : "bg-destructive/15 text-destructive border border-destructive/20"}`}>
            ...
            <div className="rounded-xl p-4" style={{ borderLeft: "4px solid hsl(var(--info))", background: "hsl(var(--info) / 0.05)" }}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: "hsl(var(--info))" }}>AI Instant Insight</span>
              </div>
              {insightLoading ? <Skeleton className="h-10 w-full" /> : <p className="text-sm leading-relaxed">{insight}</p>}
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <Button size="sm" variant="outline" className="rounded-xl text-xs" onClick={() => navWith("/diagnose/video-death")}>
                  <Skull className="mr-1 h-3.5 w-3.5" /> Autopsy
                </Button>
                <Button size="sm" variant="outline" className="rounded-xl text-xs" onClick={() => navWith("/grow/hidden-gold")}>
                  <Scissors className="mr-1 h-3.5 w-3.5" /> Shorts Scanner
                </Button>
                <Button size="sm" variant="outline" className="rounded-xl text-xs" onClick={() => navWith("/grow/comment-intelligence")}>
                  <PickaxeIcon className="mr-1 h-3.5 w-3.5" /> Mine Comments
                </Button>
                <Button size="sm" variant="outline" className="rounded-xl text-xs" onClick={() => navWith("/create/video-machine")}>
                  <Zap className="mr-1 h-3.5 w-3.5" /> Make Part 2
                </Button>
                <Button size="sm" variant="outline" className="rounded-xl text-xs" onClick={() => navWith("/create/hook-score")}>
